...
const Login = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (user.username === username && user.password === password) {
      localStorage.setItem("cyberLoggedIn", "true");
      navigate("/dashboard");
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background grid-bg relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md mx-4 relative z-10">
        ...
        <div className="bg-card border border-border rounded-lg p-6 box-glow">
          <h2 className="text-lg font-mono font-semibold text-foreground mb-1">&gt; AUTHENTICATE</h2>
          <p className="text-muted-foreground text-xs font-mono mb-6">Enter your credentials to access the system</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-primary text-primary-foreground font-mono font-semibold py-2.5 rounded hover:bg-primary/90 transition-colors text-sm">&gt; ACCESS_SYSTEM</button>
          </form>

          <p className="text-center text-xs font-mono text-muted-foreground mt-4">
            No credentials? <Link to="/register" className="text-primary hover:underline">REGISTER</Link>
          </p>
        </div>
      </motion.div>
    </div>
